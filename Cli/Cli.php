...
class Cli {
    ...
    private function help(): string {
        return
            $this->info('serve [host]:[port]') . "\t\t" . 'Start the development server (host and port are optional, default host: 127.0.0.1, default port: 8000)' . "\n" .
            $this->info('hash [value]') . "\t\t\t" . 'Hash the given value' . "\n" .
            $this->info('key') . "\t\t\t\t" . 'Generate a random encryption (secret) key' . "\n" .
            $this->info('module [name]') . "\t\t\t" . 'Create a new module' . "\n\n" .
            $this->info('migration create [name]') . "\t\t" . 'Create a new migration' . "\t\t" . '(mg create module/name for module migrations)' . "\n" .
            $this->info('migration run') . "\t\t\t" . 'Run all pending migrations' . "\t" . '(mg run -p for production)' . "\n" .
            $this->info('migration rollback') . "\t\t" . 'Rollback the last migration' . "\t" . '(mg rollback -p for production)' . "\n" .
            $this->info('migration reset') . "\t\t\t" . 'Rollback all migrations' . "\t\t" . '(mg reset -p for production)' . "\n" .
            $this->info('migration refresh') . "\t\t" . 'Reset and run all migrations' . "\t" . '(mg refresh -p for production)' . "\n" .
            $this->info('migration clear') . "\t\t\t" . 'Clear migration.json file' . "\n\n" .
            $this->info('database create') . "\t\t\t" . 'Create database from .env' . "\t" . '(db create -p for production)' . "\n" .
            $this->info('database drop') . "\t\t\t" . 'Drop database if exists' . "\t\t" . '(db drop -p for production)' . "\n" .
            $this->info('database refresh') . "\t\t" . 'Drop and create database' . "\t" . '(db refresh -p for production)' . "\n" .
            $this->info('database seed') . "\t\t\t" . 'Run database seeders' . "\t\t" . '(db seed -p for production)' . "\n\n" .
            $this->info('remove cache') . "\t\t\t" . 'Remove cache (Storage/Cache)' . "\t" . '(rm cache)' . "\n" .
            $this->info('remove log') . "\t\t\t" . 'Remove log (Storage/Logs)' . "\t" . '(rm log)' . "\n";
    }
    ...
    private function hash(string $value): string {
        ...
        if (!$hash) {
            return $this->error('✗ Hash not supported');
        }
